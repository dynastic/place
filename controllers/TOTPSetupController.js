...
exports.postTOTPSetup = function(req, res, next) {
    ...
    if(!speakeasy.totp.verify({secret: req.body.secret, encoding: 'base32', token: req.body.token})) return res.status(403).json({success: false, error: {message: "The token you inputted is not valid for the QR code on screen.", code: "invalid_totp_token"}});
