...
exports.postSignIn = (req, res, next) => {
    ...
    passport.authenticate("local", function(err, user, info) {
        ...
        if (user.twoFactorAuthEnabled()) {
            ...
            if(!speakeasy.totp.verify({ secret: user.totpSecret, encoding: 'base32', token: req.body.totpToken })) return res.status(403).json({success: false, error: {code: "invalid_totp", message: "We couldn't sign you in with that two-factor authentication token. Make sure you're entering the right code and it is updated."}});
        }
